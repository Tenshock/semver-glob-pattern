...
class Or extends ExtGlob {
  ...
  constructor(...globs: Glob[]) {
    super(
      ...globs.reduce<Glob[]>(
        (flattened, glob) =>
          flattened.concat(glob instanceof Or ? glob.getGlobs() : glob),
        [],
      ),
    );
  }
...
const numericIdentifier = new Or(
  zero,
  new Concat(positiveDigit, new ZeroOrMore(digit)),
);
const alphanumericIdentifier = new Concat(
  new ZeroOrMore(digit),
  nonDigit,
  new ZeroOrMore(identifierCharacter),
);
const prereleaseIdentifier = new Or(alphanumericIdentifier, numericIdentifier);
const dotSeparatedBuildIdentifier = new Concat(
  new OneOrMore(identifierCharacter),
  new ZeroOrMore(new Concat(dot, new OneOrMore(identifierCharacter))),
);
const build = dotSeparatedBuildIdentifier;
const dotSeparatedPreReleaseIdentifier = new Concat(
  prereleaseIdentifier,
  new ZeroOrMore(new Concat(dot, prereleaseIdentifier)),
);
...
const validSemVer = new Concat(
  versionCore,
  new ZeroOrOne(new Concat(dash, preRelease)),
  new ZeroOrOne(new Concat(plus, build)),
);

export function generateValidSemVer(): string {
  return validSemVer.get();
}
